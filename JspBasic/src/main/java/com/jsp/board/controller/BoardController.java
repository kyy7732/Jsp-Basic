...
//			response.sendRedirect("board/board_list.jsp");
			...
			// -> RequestDispatcher
			// request,response를 위임한다 -> board/board_list.jsp쪽으로
		...
		case "content":
			System.out.println("글 작성된 페이지 요청이 들어옴!");
			...
			/*
			List<BoardVO> lt = BoardRepository.getInstance().getList();
			
			request.setAttribute("boardLt", lt);
			
			RequestDispatcher ddp = request.getRequestDispatcher("board/board_content.jsp");
			ddp.forward(request, response);
			*/
		...
		case "search":
			System.out.println("글 작성자 검색 요청이 들어옴!");
