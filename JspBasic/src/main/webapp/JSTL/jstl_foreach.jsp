...
<body>
	...
	<h4>1부터 100까지의 누적합: <%=total %></h4>
	 ...
	 <c:forEach var="i" begin="1" end="100" step="1">
	 	<c:set var="total" value="${total + i}"></c:set><!-- 기존의 total을 지목  -->
	 </c:forEach>
...
<%-- 	 <h4>구구단 4단</h4>
 ...
 --%>
	...
	<c:forEach var="hang1" begin="2" end="9">
		<c:if test="${hang1 % 2 == 0 }">
		<c:forEach var="dan" begin="1" end="9">
			${hang1} x ${dan} = ${hang1*dan} <br>
		</c:forEach>
		</c:if>
		<hr>
	</c:forEach>
	...
	<c:set var="arr" value="<%=new int[] {1, 3, 5, 7, 9} %>"></c:set> <!-- EL은 생성이 안되서 자바코드로 작성  -->
